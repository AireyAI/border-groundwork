...
(function () {
  ...
  function setupTextSplit() {
    // Walk the element's child nodes, splitting text nodes into per-char or
    // per-word inline-block spans while keeping any wrapping elements (so
    // classes like `text-amber` on a child <span> are preserved).
    const splitNode = (node, mode) => {
      const out = document.createDocumentFragment();
      const makeSpan = (text) => {
        const s = document.createElement('span');
        s.style.display = 'inline-block';
        if (mode === 'words') s.style.marginRight = '0.25em';
        s.textContent = text;
        s.dataset.splitUnit = '1';
        return s;
      };
      if (node.nodeType === 3) { // TEXT_NODE
        ...
        if (mode === 'chars') {
          for (const ch of txt) {
            if (ch === ' ') out.appendChild(document.createTextNode(' '));
            else out.appendChild(makeSpan(ch));
          }
        } else {
          const parts = txt.split(/(\s+)/);
          for (const p of parts) {
            if (!p) continue;
            if (/^\s+$/.test(p)) out.appendChild(document.createTextNode(p));
            else out.appendChild(makeSpan(p));
          }
        }
      } else if (node.nodeType === 1) { // ELEMENT_NODE
        const clone = node.cloneNode(false);
        Array.from(node.childNodes).forEach(child => {
          clone.appendChild(splitNode(child, mode));
        });
    ...
    document.querySelectorAll('[data-split]').forEach(el => {
      ...
      if (mode === 'chars' || mode === 'words') {
        ...
        Array.from(el.childNodes).forEach(child => {
          frag.appendChild(splitNode(child, mode));
        });
        el.innerHTML = '';
        el.appendChild(frag);

        const units = el.querySelectorAll('[data-split-unit="1"]');
        gsap.from(units, {
          y: mode === 'chars' ? '110%' : '100%',
          opacity: 0,
          duration: mode === 'chars' ? 0.6 : 0.7,
          ease: mode === 'chars' ? 'power4.out' : 'power3.out',
